...
class GFocalHead(AnchorHead):
    ...
    def forward_single(self, x, scale):
        ...
        prob_topk1 = F.softmax(prob_topk, dim=2)
        prob_topk2, _ = prob_topk1.topk(1, dim=2)
        if self.add_mean:
            stat = torch.cat([prob_topk, prob_topk2, torch.sqrt(prob_topk *prob_topk1)],
                             dim=2)
        else:
            stat = prob_topk
        ...
        return cls_score, bbox_pred, cls_feat, quality_score
